...
BEGIN
    ...
    SET a.STATUS = CASE
            WHEN DAYOFWEEK(a.attendance_date) IN (r.weekend_day1, r.weekend_day2)
                THEN 'Weekend'
            ...
            WHEN a.check_in_time IS NOT NULL
             AND a.check_out_time IS NULL THEN
                CASE
                    WHEN a.check_in_time <= DATE_ADD(
                        CONCAT(a.attendance_date, ' ', r.work_start_hour),
                        INTERVAL r.late_after_minutes MINUTE
                    )
                        THEN 'Waiting Punch-out'
                    ELSE 'Arrive Late'
                END
            -- Punch-out
            WHEN a.check_out_time IS NOT NULL THEN
                CASE
                    WHEN a.check_out_time >= DATE_ADD(
                        CONCAT(a.attendance_date, ' ', r.work_end_hour),
